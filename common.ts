...
import { readableStreamToString, IntelliHrError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new IntelliHrError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new IntelliHrError(e, e.response?.data)
                }
                if (innerError instanceof IntelliHrError) {
                    // Got "IntelliHrError" from the above try block
                    throw innerError;
                }
